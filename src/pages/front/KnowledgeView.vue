    ...
    <div id="mycontent" class="col row justify-start">
      ...
      <div class="col-12 q-mt-md q-pl-lg" style="width: 100%;">
        <q-breadcrumbs class="text-wrap">
          ...
          <q-breadcrumbs-el label="專欄文章" icon="fa-brands fa-readme" to="/knowledges" />
          <q-breadcrumbs-el label="靈學知識" icon="fa-regular fa-lightbulb" to="/knowledges" />
          <q-breadcrumbs-el :label="knowledge.title" icon="fa-regular fa-newspaper" style="display: inline-block;" />
        </q-breadcrumbs>
      </div>
      ...
      <div class="col-12 row justify-start q-mt-lg q-mb-xl q-pl-lg" style="width: 100%;">
        ...
        <div class="col-12 row">
          ...
          <div class="col">
            ...
            <div class="text-secondary text-subtitle1 spacing-h6 q-my-sm">{{ new
              Date(knowledge.date).toLocaleString() }}
            </div>
            <!-- 消息標題 -->
            <div class="text-accent text-h6 text-xl-h5 spacing-h7 q-mt-md q-pr-lg text-wrap">{{ knowledge.title }}
            </div>
          </div>
        </div>
        <!-- 內文區 -->
        <div class="q-mt-lg" style="height: auto;">
          <!-- 消息內容 -->
          <p v-html="knowledge.content" class="col-12 col-xl-6 text-xl-h6 text-secondary text-justify q-mt-lg q-pr-lg">
          </p>
        </div>
      </div>
    </div>
...
const init = async () => {
  try {
    const { data } = await api.get('/knowledges/' + route.params.id)
    knowledge._id = data.result._id
    knowledge.title = data.result.title
    knowledge.content = data.result.content
    knowledge.date = data.result.date
  } catch (error) {
    Swal.fire({
      icon: 'error',
      title: '失敗',
